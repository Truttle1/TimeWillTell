...
public class FlairmerGOverworld extends NPC{
	...
	boolean idle1 = false;
	boolean idle2 = false;
	public FlairmerGOverworld(int x, int y, Game window, OverworldMode om, int amount) {
		super(window,x,y, om);
		this.window = window;
		...
		this.amount = amount;
		this.currentAnimation = BattleAnimation.flareGreenIdle;
	}
		...
		for(int i=0; i<window.overworldMode.objects.size();i++)
		{
			if(Global.talking == 0 && !Fade.running && window.overworldMode.objects.get(i).id == ObjectId.Player)
			{
				if(!defeated && this.getBounds().intersects(window.overworldMode.objects.get(i).getBounds()))
				{
					if(amount == 1)
					{
						window.battleMode.addMonster(new FlairmerGreen(window,720,270,window.battleMode));
					}
					if(amount == 2)
					{
