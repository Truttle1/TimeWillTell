...
public class Gravel extends Grass{
	...
	Color clr;
	public Gravel(int x, int y, Game window, OverworldMode om) {
		super(x, y, window, om);
		currentAnimation = OverworldAnimation.gravel;
		makesDirt = false;
		id = ObjectId.Grass;
		clr = Color.black;
	}
	public Gravel(int x, int y, Game window, OverworldMode om, BufferedImage[] img, Color clr) {
		super(x, y, window, om);
			...
			if(topLine && bottomLine && leftLine && rightLine)
			{
				for(int i=0; i<om.objects.size();i++)
				{
					if(om.objects.get(i) != null && om.objects.get(i).id == ObjectId.Grass)
					{
						...
						if(g != null && g != this && g instanceof Gravel)
						{
